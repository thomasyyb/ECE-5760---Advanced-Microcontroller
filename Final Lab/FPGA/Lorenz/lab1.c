...
int main(void)
{
	...
	*pio_lclock_write_ptr = 1;
	...
	while(1) 
	{
		...
		// max_x = (max_x > x_value) ? max_x : x_value ;
		// max_y = (max_y > y_value) ? max_y : y_value ;
		// max_z = (max_z > z_value) ? max_z : z_value ;

		// min_x = (min_x < x_value) ? min_x : x_value ;
		// min_y = (min_y < y_value) ? min_y : y_value ;
		// min_z = (min_z < z_value) ? min_z : z_value ;

		// printf("max_x = %d, max_y = %d, max_z = %d\n", max_x, max_y, max_z);
		// printf("min_x = %d, min_y = %d, min_z = %d\n\n", min_x, min_y, min_z);


		// VGA_PIXEL((x_value+20)+160, (y_value+30)+140, colors[color_index]);
		// VGA_PIXEL((x_value+20)+460, (z_value)+90, colors[color_index]);
		// VGA_PIXEL((y_value+30)+300, (z_value)+280, colors[color_index]);
		VGA_line((x_old+20)+160, (y_old+30)+140, (x_value+20)+160, (y_value+30)+140, colors[color_index]);
		VGA_line((x_old+20)+460, (z_old)+90, (x_value+20)+460, (z_value)+90, colors[color_index]);
		VGA_line((y_old+30)+300, (z_old)+280, (y_value+30)+300, (z_value)+280, colors[color_index]);
