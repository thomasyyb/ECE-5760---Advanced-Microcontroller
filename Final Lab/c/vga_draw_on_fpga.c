...
/// DE1 computer
/// compile with
/// gcc 2_circle.c -o gr -O2 -lm -lpthread
///
///////////////////////////////////////
#include <stdio.h>
#include <stdlib.h>
...
// PIO phasor
#define PIO_PHASOR_SIN_OUT_1_OFFSET   0x20
#define PIO_PHASOR_SIN_OUT_2_OFFSET   0x30
#define PIO_PHASOR_COS_OUT_1_OFFSET   0x40
#define PIO_PHASOR_COS_OUT_2_OFFSET   0x50
...
volatile unsigned int * pio_phasor_sin_out_1_read_ptr = NULL ;
volatile unsigned int * pio_phasor_sin_out_2_read_ptr = NULL ;
volatile unsigned int * pio_phasor_cos_out_1_read_ptr = NULL ;
volatile unsigned int * pio_phasor_cos_out_2_read_ptr = NULL ;
...
int main(void)
{
	...
	pio_phasor_sin_out_1_read_ptr =(unsigned int *)(h2p_pio_virtual_base+PIO_PHASOR_SIN_OUT_1_OFFSET);
	pio_phasor_sin_out_2_read_ptr =(unsigned int *)(h2p_pio_virtual_base+PIO_PHASOR_SIN_OUT_2_OFFSET);
	pio_phasor_cos_out_1_read_ptr =(unsigned int *)(h2p_pio_virtual_base+PIO_PHASOR_COS_OUT_1_OFFSET);
	pio_phasor_cos_out_2_read_ptr =(unsigned int *)(h2p_pio_virtual_base+PIO_PHASOR_COS_OUT_2_OFFSET);
	...
	*pio_lclock_write_ptr = 1;
	...
	int phasor_1_cos, phasor_2_cos, phasor_1_sin, phasor_2_sin;
	int phasor_1_cos_old, phasor_2_cos_old, phasor_1_sin_old, phasor_2_sin_old;

	phasor_1_sin = (int)(*pio_phasor_sin_out_1_read_ptr) ;
	phasor_2_sin = (int)(*pio_phasor_sin_out_2_read_ptr) ;
	phasor_1_cos = (int)(*pio_phasor_cos_out_1_read_ptr) ;
	phasor_2_cos = (int)(*pio_phasor_cos_out_2_read_ptr) ;

	printf("phasor_1_sin = 0x%x, phasor_1_cos = 0x%x\n", phasor_1_sin, phasor_1_cos);
	printf("phasor_2_sin = 0x%x, phasor_2_cos = 0x%x\n", phasor_2_sin, phasor_2_cos);

	phasor_1_sin = phasor_1_sin << 12;
	phasor_2_sin = phasor_2_sin << 12;
	phasor_1_cos = phasor_1_cos << 12;
	phasor_2_cos = phasor_2_cos << 12;

	phasor_1_sin = phasor_1_sin >> 22;
	phasor_2_sin = phasor_2_sin >> 22;
	phasor_1_cos = phasor_1_cos >> 22;
	phasor_2_cos = phasor_2_cos >> 22;
...
// create threads
	...
	while(1) 
	{
		...
		phasor_1_sin_old = phasor_1_sin;
		phasor_2_sin_old = phasor_2_sin;
		phasor_1_cos_old = phasor_1_cos;
		phasor_2_cos_old = phasor_2_cos;
		...
		VGA_line(center_x, 					    center_y, 
				 center_x + phasor_1_sin_old,   center_y + phasor_1_cos_old, colors[10]);
		...
		// Draw a new arrow 
		VGA_line(center_x, 					    center_y, 
				 center_x + phasor_1_sin, 		center_y + phasor_1_cos, colors[7]); // inner arrow (cyan)
		VGA_line(center_x + phasor_1_sin, 		center_y + phasor_1_cos, // outer arrow (magenta)
				center_x + phasor_1_sin + phasor_2_sin, center_y + phasor_1_cos + phasor_2_cos, colors[8]);
		// Draw the shape 
		VGA_line(phasor_sum_sin_old + center_x, phasor_sum_cos_old + center_y,
				 phasor_sum_sin     + center_x, phasor_sum_cos     + center_y, colors[6]); // yellow 
